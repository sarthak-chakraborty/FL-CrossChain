...
func main() {
	...
	result, err = contract.SubmitTransaction("CreateAsset", "asset2", "yellow", "5", "Tom", "1300")
	if err != nil {
		log.Fatalf("Failed to Submit transaction: %v", err)
	}
	log.Println(string(result))
	...
	result, err = contract.EvaluateTransaction("ReadAsset", "asset2")
	if err != nil {
	...
	result, err = contract.EvaluateTransaction("AssetExists", "asset2")
	if err != nil {
		log.Fatalf("Failed to evaluate transaction: %v\n", err)
	}
	log.Println(string(result))
	...
	_, err = contract.SubmitTransaction("TransferAsset", "asset2", "Tom")
	if err != nil {
		log.Fatalf("Failed to Submit transaction: %v", err)
	}
