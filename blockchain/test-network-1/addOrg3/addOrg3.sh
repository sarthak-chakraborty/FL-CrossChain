...
function generateOrg3Definition() {
  ...
  echo "##########################################################"
   export FABRIC_CFG_PATH=$PWD
   set -x
   configtxgen -printOrg Org3MSP > ../organizations/peerOrganizations/org3-net1.example.com/org3-net1.json
   res=$?
   { set +x; } 2>/dev/null
   if [ $res -ne 0 ]; then
...
function addOrg3 () {
  ...
  if [ ! -d "../organizations/peerOrganizations/org3-net1.example.com" ]; then
    generateOrg3
