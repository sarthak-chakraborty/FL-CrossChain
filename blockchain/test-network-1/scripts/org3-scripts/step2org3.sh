...
joinChannelWithRetry() {
  ...
  verifyResult $res "After $MAX_RETRY attempts, peer0.org${ORG}-net1 has failed to join channel '$CHANNEL_NAME' "
}


echo "Fetching channel config block from orderer..."
set -x
peer channel fetch 0 $CHANNEL_NAME.block -o orderer-net1.example.com:7050 --ordererTLSHostnameOverride orderer-net1.example.com -c $CHANNEL_NAME --tls --cafile $ORDERER_CA >&log.txt
res=$?
{ set +x; } 2>/dev/null
cat log.txt
